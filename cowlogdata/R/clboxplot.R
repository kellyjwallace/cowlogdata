...
clboxplot <- function(dataname, factor, factorname){
  ...
  if(factor == FALSE){
    ...
    for(i in 1:(length(collist))){
      ...
      colnames(temp)<-"value"
      p<-ggplot(temp, aes(x="", y=value)) +
        geom_boxplot()+
        geom_dotplot(binaxis='y', stackdir='center', dotsize=1)+
        theme_classic()+
        labs(title="all data", x="", y=paste(collist[i],"--mean:", avg))+
  ...
  if(factor == TRUE){
    ...
    for(i in 1:(length(collist))){
      ...
      if(length(unique(omitted$factor))>1 & length(unique(omitted$factor))<3){
        factor_list<-unique(temp$factor)
        templist<-NA
        k<-1
        for (k in 1:length(factor_list)){
          lengthvalue<-omitted %>% filter(factor == factor_list[k]) %>% nrow()
        ...
        print(paste(factorname,collist[[i]],  "p value (t-test) = ", round(p_value_t, 4)))
        if(p_value_t<0.05){
          p<-ggplot(temp, aes(x=factor, y=value)) +
            geom_boxplot()+
            geom_dotplot(binaxis='y', stackdir='center', dotsize=1)+
            theme_classic()+
            labs(title=(paste(collist[i], "by", factorname)), x="", y=collist[i])+
            theme(aspect.ratio = 3)+
            theme(axis.text.x = element_text(angle = 90))+
            annotate(geom = 'text', label = paste("p value (t-test) =
                                                  ", round(p_value_t, 4)), x = -Inf, y = Inf, hjust = 0, vjust = 1)
          plot(p)
        }}
      ...
      if(length(unique(omitted$factor))>2){
        ...
        print(paste(factorname,collist[[i]],  "p value (anova) = ", round(p_value_anova[1], 4)))
     ...
     ", round(p_value_anova[1], 4)), x = -Inf, y = Inf, hjust = 0, vjust = 1)
          plot(p)}}}}}
