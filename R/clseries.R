...
clseries <- function(pathtofile, zonename, seglength, factor, factorindex, factorname){
  ...
  seriesdata<-as.data.frame(matrix(0, ncol = (length(zones)*10)+1, nrow = length(file_list)))
  i<-1
  j<-1
  listnames<-"filename"
  for(j in 1:length(zones)){
    for(i in 1:10){
      listnames <- append(listnames, paste("segment", i, "-", zones[j],sep = ""))}}
  ...
  for(i in 1:length(file_list)){
    ...
    for (j in 1:length(zones)){
      for (k in 1:10){
        seriesdata[i,paste("segment", k, "-", zones[j], sep = "")] <-0
        temp<-eval(parse(text = paste("segment", k, sep= ""))) %>% filter(code == zones[j]) %>% select(time.difference)
        if(nrow(temp)>0){
          seriesdata[i,paste("segment", k, "-", zones[j], sep = "")] <- temp %>% sum()/seglength}}}}

  fullframe<-as.data.frame(matrix(0, ncol = length(zones)+1, nrow = 10))
  ...
  if(factor == FALSE){
    ...
    for (i in 1:10){
      ...
      tempseries<-seriesdata[names(seriesdata[grep(tempcollist,names(seriesdata))])]
      for (j in 1:length(zones)){
        tempserieszone<- tempseries[names(tempseries[grep(zones[j],names(tempseries))])]
        fullframe[i,zones[j]]<-0
        if(nrow(unique(tempserieszone))>1){
          meanzone<-sapply(tempserieszone, mean, na.rm = TRUE)[1] *100
          fullframe[i,zones[j]]<-as.numeric(meanzone)
        }}}

    segs <- rep(c("1","2","3","4","5","6","7","8","9","t10"), length(zones))
    i<-1
    zoneschart<-NA
    ...
    p<-ggplot(data, aes(fill=zoneschart, y=percent, x=segs)) +
      geom_bar(position="stack", stat="identity") +
      ggtitle(paste("location over time, all data
                    seconds per segment =",seglength))+
      theme_classic()+
      labs(fill = "zones",  x= "time segment", y = "proportion of time in area")+
  ...
  if (factor == TRUE){
    ...
    for (k in 1:length(unique(seriesdata[,factorname]))){
      ...
      seriesdatafactor<-seriesdata %>% filter(seriesdata[factorname] == tempfactorname)

      i<-1
      j<-1
      for (i in 1:10){
        tempcollist<-paste("segment", i, "-",sep = "")
        tempseries<-seriesdatafactor[names(seriesdatafactor[grep(tempcollist,names(seriesdatafactor))])]
        for (j in 1:length(zones)){
          tempserieszone<- tempseries[names(tempseries[grep(zones[j],names(tempseries))])]
          fullframe[i,zones[j]]<-0
          if(nrow(unique(tempserieszone))>1){
