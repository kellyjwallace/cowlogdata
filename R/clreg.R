      ...
      for (j in 1:length(collist)){
        ...
        if(ncol(temp)>1){
          if(nrow(na.omit(temp))>0){
            ...
            print(paste(measure1, measure2, "p value = ", round(p_value, 4)))
            if(is.na(p_value)== TRUE){p_value<-100}
            if(p_value>0 & p_value<0.05){
              p <- ggplot(temp, aes(measure1, measure2)) +
                geom_point(size = 4) +
                geom_smooth(method = "lm", se = FALSE, colour="black") +
                theme(text = element_text(size=10)) +
                labs(x= measure2, y=measure1) +
                theme_classic()+
                annotate(geom = 'text', label = paste("p value = ", round(p_value, 4)), x = -Inf, y = Inf, hjust = 0, vjust = 1)
              plot(p)
            }}}}}}
  ...
  if(factor == TRUE){
    ...
    for (i in 1:length(collist)){
      for (j in 1:length(collist)){
        measure1<-collist[i]
        measure2<-collist[j]
        temp<-data %>% select(collist[i], collist[j], factorname)
        if(ncol(temp)>2){
          if(nrow(na.omit(temp))>0){
            colnames(temp)<-c("measure1", "measure2", "category")
            temp$measure1<-as.numeric(temp$measure1)
            temp$measure2<-as.numeric(temp$measure2)
            ...
            if(length(unique(omitted$category))>1){
              mod<-lm(temp$measure2 ~ temp$measure1*temp$category)
              p_value<-tidy(mod)[[5]][4]}
            print(paste(measure1, measure2, "p value (interaction effect) = ", round(p_value, 4)))
            if(is.na(p_value)== TRUE){p_value<-100}
            if(p_value>0 & p_value<0.05){
              p <- ggplot(temp, aes(measure1, measure2, color = category)) +
                geom_point(size = 4) +
                ...
                annotate(geom = 'text', label = paste("p value (interaction effect) = ", round(p_value, 4)), x = -Inf, y = Inf, hjust = 0, vjust = 1)
              plot(p)
            }}}}}}}
